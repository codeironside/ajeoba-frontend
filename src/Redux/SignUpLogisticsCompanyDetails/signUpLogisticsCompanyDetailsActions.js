...
export const signUpLogisticsCompanyDetails = (data, navigate) => {
  return (dispatch) => {
    call({
      method: "post",
      endpoint: "api/users/profile/logisticsCompanyDetails",
      payload: data,
      dispatch,
    })
      .then((res) => {
        if (res.status === 200) {
          showToast(
            "Logistics Company details updated successfully",
            "success"
          );
          const userData = JSON.stringify(res.body.data.userDetails);
          const accountData = JSON.stringify(
            res.body.data.logisticsCompanyDetails
          );
