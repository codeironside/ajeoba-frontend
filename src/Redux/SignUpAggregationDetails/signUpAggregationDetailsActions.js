...
export const signUpAggregatorDetails = (data, navigate) => {
  return (dispatch) => {
    call({
      method: "post",
      endpoint: "api/users/profile/aggregatorDetails",
      payload: data,
      dispatch,
    })
      .then((res) => {
        if (res.status === 200) {
          showToast("Aggregator details updated successfully", "success");
