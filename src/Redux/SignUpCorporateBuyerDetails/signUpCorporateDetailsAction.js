...
export const signUpCorporateBuyerDetails = (data, navigate) => {
  
  return (dispatch) => {
    call({
      method: "post",
      endpoint: "api/users/profile/corporateBuyerDetails",
      payload: data,
      dispatch,
    })
      .then((res) => {
        if (res.status === 200) {
          showToast("Corporate Buyer details updated successfully", "success");
          const userData = JSON.stringify(res.body.data.userDetails);
          const accountData = JSON.stringify(
            res.body.data.corporateBuyerDetails
          );
