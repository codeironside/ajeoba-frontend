...
export const signUpAssociationDetails = (data, navigate) => {
  return (dispatch) => {
    call({
      method: "post",
      endpoint: "api/users/profile/associationDetails",
      payload: data,
      dispatch,
    })
      .then((res) => {
        if (res.status === 200) {
          showToast("Association details updated successfully", "success");
          const userData = JSON.stringify(res.body.data.userDetails);
          const accountData = JSON.stringify(res.body.data.associationDetails);
